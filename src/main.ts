...
// Global typing reference for Lucide Icons
...
function getFolderIconSVG(size: number = 64): string {
  return `
...
function getCysterPageHTML(project: Project): string {
  return `
...
function getLMSPageHTML(project: Project): string {
  return `
...
function getViCaPageHTML(project: Project): string {
  return `
...
function getProjectPageHTML(project: Project): string {
  if (project.id === 'vica') {
...
function openWindow(id: string, type: 'project' | 'about') {
...
/* Dragging Engine */
function setupDragging(winEl: HTMLElement, headerEl: HTMLElement, id: string) {
  let startX = 0;
  let startY = 0;
  let startLeft = 0;
  let startTop = 0;
...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────── */
const spotlightSearchItems: SearchItem[] = [
  { id: "about", type: "about", title: "About Me", subtitle: `${PROFILE.title} · ${PROFILE.location}`, action: () => openWindow("about", "about") },
  ...PROJECTS.map(p => ({ id: p.id, type: "project" as const, title: p.name, subtitle: p.tagline, action: () => openWindow(p.id, "project") })),
...
function toggleSpotlight(open: boolean) {
  state.searchOpen = open;
  const spotlightEl = document.getElementById('spotlight-search');
  const inputEl = document.getElementById('spotlight-input') as HTMLInputElement;
...
function renderSpotlightResults(query: string) {
...
function selectDesktopIcon(id: string | null) {
...
function setupMobileProfileHeader() {
  const photo = document.getElementById('mobile-profile-photo') as HTMLImageElement;
...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────── */
const MENUS: Record<string, Array<{ label: string; shortcut?: string; sep?: boolean; disabled?: boolean; action?: () => void }>> = {
  Finder: [
...
function renderDropdown(menuKey: string, leftPos: number) {
  const activeDropdown = document.getElementById('active-dropdown');
  const overlay = document.getElementById('dropdown-overlay');
  
  if (!activeDropdown || !overlay) return;
...
function renderDock() {
  ...
  if (!minimizedContainer || !minimizedDivider) return;
